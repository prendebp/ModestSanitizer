...
namespace ModestSanitizer
{
    /// <summary>
    ///  Blacklist = 7
    ///  Throw exception on blacklist values (for monitoring/cleansing, un-advised by itself, whitelist is better)
    ...
    public class Blacklist
    {
        ...
        public Blacklist(SaniCore saniCore)
        {
            SaniCore = saniCore;

            TruncateLength = 10;
            SaniType = SaniTypes.Blacklist;
        }
        ...
        public static List<string> GenerateHexAndEscapeSeqBlacklist()
        {
        ...
        public static List<string> GenerateCommonBlacklist()
        {
            ...
            {  
                ...
                @"\u2B7F",//replace vertical tab with empty string
                          //commonBlacklist.Add(@"\u005C"); //replace reverse solidus or backslash with empty string. File path may allow.
                @"\u200B", //replace zero-width space character with empty string
                @"\u2009", //replace thin space with empty string
                @"\u007F", //replace delete with empty string
                           //commonBlacklist.Add(@"\u007E"); //replace tilde with empty string. File path may allow.
        ...
        /// <summary>
        /// Review - compare string to check against blacklist value while ignoring case. Returns true if any issue or blacklist match found. stringToCheck will be cleansed.
        /// </summary>
        /// <param name="stringToCheck"></param>
        /// <returns></returns>   
        public bool? ReviewIgnoreCaseUsingASCII(ref string stringToCheck, List<string> blacklistValues, int lengthToTruncateTo, bool checkForHexChars = true, bool checkForCommonMaliciousChars = true)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        List<string> hexBlacklist = Blacklist.GenerateHexAndEscapeSeqBlacklist();
                    ...
                    if (hasOtherMaliciousChars)
                    {
                        ...
                        if (hasCommonMaliciousChars)
                        {
                            throw new Exception("StringToCheck contains a common malicious character and a blacklist value.");
                        }
                        else
                        {
            ...
            {
                tmpResult = true;
                SaniExceptionHandler.TrackOrThrowException(TruncateLength, SaniType, SaniCore, "Blacklist: ", "Issue with Blacklist ReviewIgnoreCase method", stringToCheck, ex);
            }
            return tmpResult;
        }
